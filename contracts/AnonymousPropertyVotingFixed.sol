...
/**
 * @title AnonymousPropertyVotingFixed
 * @notice Fixed version that properly handles proposals with zero votes
 */
contract AnonymousPropertyVotingFixed is SepoliaConfig {
    ...
    /**
     * @notice END PROPOSAL - FIXED VERSION
     * @dev Properly handles proposals with zero votes
     */
        ...
        onlyAfterVotingEnds(proposalId)
    {
        ...
        // ✅ FIX: Check if there are any votes
        if (proposal.voters.length == 0) {
            // No votes - directly finalize the proposal
            proposal.yesVotes = 0;
            proposal.noVotes = 0;
            ...
            // Increment to next proposal
            currentProposal++;
        } else {
            // Has votes - use FHE decryption
